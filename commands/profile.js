    ...
    sql.get(`SELECT * FROM scores WHERE guildId ="${message.guild.id}"`).then(row2 => {
        ...
        if (message.mentions.users.size < 1) {
            sql.get(`SELECT * FROM profiles WHERE guildId ="${message.guild.id}" AND userId ="${message.author.id}"`).then(row => {
              ...
              var user = message.author || message.content.includes("<@" + client.user.id +">") || message.content.includes("<@!" + client.user.id +">");
              
                 let desc = db.fetch(`desc_${message.guild.id}_${user.id}`); {
                     if (desc === null) desc = "Añade Una Descripción";
                }
                    const embed = new Discord.RichEmbed()
                        .setColor(0x00A2E8)
                        .setTitle(" Profile info For: " + message.author.username + " ")
                        .setThumbnail(message.author.avatarURL)
                        .addField("Your current level: ", row.level, true)
        ...
        } else if (message.author || message.content.includes("<@" + client.user.id +">") || message.content.includes("<@!" + client.user.id +">")) {
               /* let curxp = row.xp;
                let curlvl = row.level
                let nxtLVL = curlvl * 200;
                let dif = nxtLVL - curxp; */
