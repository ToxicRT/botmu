...
exports.run = (client, message, args) => { 
    sql.get(`SELECT * FROM scores WHERE guildId ="${message.guild.id}"`).then(row => {
     ...
     const embed10 = new Discord.RichEmbed()
        ...
        .setDescription("Descripcion: " + "Usado para tener la lista de comandos.");
        ...
        if (numberpicked === 1) {
            ...
            message.author.send(embed).then(() => message.channel.send("Los comandos han sido envias a tus mensajes privados.")).catch(() => message.channel.send("Ha ocurrido un error enviando los comandos, si tienes los mensajes privados desactivados `Activalo`"))
